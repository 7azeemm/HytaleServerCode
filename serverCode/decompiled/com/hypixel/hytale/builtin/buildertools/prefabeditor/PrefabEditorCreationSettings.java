...
JsonAssetWithMap<String, DefaultAssetMap<String, PrefabEditorCreationSettings>> {
    ...
    public static final AssetBuilderCodec<String, PrefabEditorCreationSettings> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(PrefabEditorCreationSettings.class, PrefabEditorCreationSettings::new, Codec.STRING, (builder, id) -> {
        builder.id = id;
    }, builder -> builder.id, (builder, data) -> {
        builder.data = data;
    }, builder -> builder.data).append(new KeyedCodec<PrefabRootDirectory>("RootDirectory", new EnumCodec<PrefabRootDirectory>(PrefabRootDirectory.class)), (o, rootDirectory) -> {
        o.prefabRootDirectory = rootDirectory;
    }, o -> o.prefabRootDirectory).add()).append(new KeyedCodec<T[]>("UnprocessedPrefabPaths", new ArrayCodec<String>(Codec.STRING, String[]::new)), (o, unprocessedPrefabPaths) -> {
        o.unprocessedPrefabPaths = List.of(unprocessedPrefabPaths);
    }, o -> (String[])o.unprocessedPrefabPaths.toArray(String[]::new)).add()).append(new KeyedCodec<Integer>("PasteYLevelGoal", Codec.INTEGER), (o, pasteYLevelGoal) -> {
        o.pasteYLevelGoal = pasteYLevelGoal;
    }, o -> o.pasteYLevelGoal).add()).append(new KeyedCodec<Integer>("BlocksBetweenEachPrefab", Codec.INTEGER), (o, blocksBetweenEachPrefab) -> {
        o.blocksBetweenEachPrefab = blocksBetweenEachPrefab;
    }, o -> o.blocksBetweenEachPrefab).add()).append(new KeyedCodec<WorldGenType>("WorldGenType", new EnumCodec<WorldGenType>(WorldGenType.class)), (o, worldGenType) -> {
        o.worldGenType = worldGenType;
    }, o -> o.worldGenType).add()).append(new KeyedCodec<Integer>("BlocksAboveSurface", Codec.INTEGER), (o, blocksAboveSurface) -> {
        o.blocksAboveSurface = blocksAboveSurface;
    }, o -> o.blocksAboveSurface).add()).append(new KeyedCodec<PrefabStackingAxis>("PrefabStackingAxis", new EnumCodec<PrefabStackingAxis>(PrefabStackingAxis.class)), (o, stackingAxis) -> {
        o.stackingAxis = stackingAxis;
    }, o -> o.stackingAxis).add()).append(new KeyedCodec<PrefabAlignment>("PrefabAlignment", new EnumCodec<PrefabAlignment>(PrefabAlignment.class)), (o, alignment) -> {
        o.alignment = alignment;
    }, o -> o.alignment).add()).append(new KeyedCodec<Boolean>("RecursiveSearch", Codec.BOOLEAN), (o, recursive) -> {
        o.recursive = recursive;
    }, o -> o.recursive).add()).append(new KeyedCodec<Boolean>("LoadChildren", Codec.BOOLEAN), (o, loadChildren) -> {
        o.loadChildren = loadChildren;
    }, o -> o.loadChildren).add()).append(new KeyedCodec<Boolean>("LoadEntities", Codec.BOOLEAN), (o, loadEntities) -> {
    ...
    }, o -> o.enableWorldTicking).add()).append(new KeyedCodec<PrefabRowSplitMode>("RowSplitMode", new EnumCodec<PrefabRowSplitMode>(PrefabRowSplitMode.class)), (o, rowSplitMode) -> {
        o.rowSplitMode = rowSplitMode;
    }, o -> o.rowSplitMode).add()).append(new KeyedCodec<String>("Environment", Codec.STRING), (o, environment) -> {
        o.environment = environment;
    }, o -> o.environment).addValidator(Environment.VALIDATOR_CACHE.getValidator()).add()).append(new KeyedCodec<String>("GrassTint", Codec.STRING), (o, grassTint) -> {
        o.grassTint = grassTint;
    }, o -> o.grassTint).add()).build();
    ...
    private final transient List<Path> prefabPaths = new ObjectArrayList<Path>();
    private List<String> unprocessedPrefabPaths = new ObjectArrayList<String>();
    ...
    public static DefaultAssetMap<String, PrefabEditorCreationSettings> getAssetMap() {
        return PrefabEditorCreationSettings.getAssetStore().getAssetMap();
    }
    ...
    PrefabEditorCreationContext finishProcessing(Player editor, PlayerRef playerRef, boolean creatingNewPrefab) {
        ...
        for (String string : this.unprocessedPrefabPaths) {
            String relativePath;
            Path rootPath;
            String string4 = StringUtil.stripQuotes(string);
            string4 = string4.replace('/', File.separatorChar);
            string4 = string4.replace('\\', File.separatorChar);
            if (!SingleplayerModule.isOwner(playerRef) && !string4.isEmpty() && Path.of(string4, new String[0]).isAbsolute()) {
            ...
            if (string4.endsWith(File.separator)) {
                rootPath = this.resolveRootPathForInput(string4);
                relativePath = this.getRelativePathForInput(string4);
            ...
            if (!PrefabEditorCreationSettings.stringEndsWithPrefabPath(string4)) {
                String string5 = string4 + ".prefab.json";
            }
            try {
                void var5_5;
                rootPath = this.resolveRootPathForInput((String)var5_5);
                relativePath = this.getRelativePathForInput((String)var5_5);
        ...
        if (!creatingNewPrefab) {
            for (Path path2 : this.prefabPaths) {
                if (Files.exists(path2, new LinkOption[0])) continue;
                this.player.sendMessage(Message.translation("server.commands.editprefab.load.error.prefabNotFound").param("path", path2.toString()));
                return null;
            }
        }
        if (this.prefabPaths.isEmpty()) {
            Message header = Message.translation("server.commands.editprefab.noPrefabsInPath");
            Set<Message> set = this.unprocessedPrefabPaths.stream().map(p -> this.prefabRootDirectory.getPrefabPath().resolve((String)p)).map(Path::toString).map(Message::raw).collect(Collectors.toSet());
            this.player.sendMessage(MessageFormat.list(header, set));
            return null;
        }
        return this;
    }
    ...
    private Path resolveRootPathForInput(@Nonnull String inputPath) {
        ...
        for (PrefabStore.AssetPackPrefabPath packPath : PrefabStore.get().getAllAssetPrefabPaths()) {
            if (!packPath.getDisplayName().equals(firstComponent)) continue;
