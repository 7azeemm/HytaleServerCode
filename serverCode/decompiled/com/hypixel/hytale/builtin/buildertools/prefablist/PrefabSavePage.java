...
extends InteractiveCustomUIPage<PageData> {
    ...
    public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, @Nonnull Store<EntityStore> store) {
        ...
        eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#MainPage #SaveButton", new EventData().append("Action", Action.Save.name()).append("@Name", "#MainPage #NameInput.Value").append("@Entities", "#MainPage #Entities #CheckBox.Value").append("@Empty", "#MainPage #Empty #CheckBox.Value").append("@Overwrite", "#MainPage #Overwrite #CheckBox.Value").append("@FromClipboard", "#MainPage #FromClipboard #CheckBox.Value").append("@UsePlayerAnchor", "#MainPage #UsePlayerAnchor #CheckBox.Value").append("@ClearSupport", "#MainPage #ClearSupport #CheckBox.Value"));
            ...
            case 0: {
                ...
                BuilderToolsPlugin.addToQueue(playerComponent, this.playerRef, (r, s, componentAccessor) -> {
                    if (data.fromClipboard) {
                        s.save((Ref<EntityStore>)r, data.name, true, data.overwrite, data.clearSupport, targetPack, (ComponentAccessor<EntityStore>)componentAccessor);
                    } else {
                        s.saveFromSelection((Ref<EntityStore>)r, data.name, true, data.overwrite, data.entities, data.empty, playerAnchor, data.clearSupport, targetPack, (ComponentAccessor<EntityStore>)componentAccessor);
                    }
                });
                break;
            }
    ...
    protected static class PageData {
        ...
        }, o -> o.clearSupport).add()).append(new KeyedCodec<String>("Pack", Codec.STRING), (o, s) -> {
