...
public class PastePrefabOperation
extends SequenceBrushOperation {
    public static final BuilderCodec<PastePrefabOperation> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(PastePrefabOperation.class, PastePrefabOperation::new).append(new KeyedCodec<String>("PrefabListAssetName", Codec.STRING), (op, val) -> {
        op.prefabListAssetId = val;
    }, op -> op.prefabListAssetId).documentation("The name of a PrefabList asset").add()).documentation("Paste a prefab at the origin+offset point")).build();
    @Nullable
    ...
    public boolean modifyBlocks(@Nonnull Ref<EntityStore> ref, @Nonnull BrushConfig brushConfig, BrushConfigCommandExecutor brushConfigCommandExecutor, BrushConfigEditStore edit, int x, int y, int z, ComponentAccessor<EntityStore> componentAccessor) {
        PrefabListAsset prefabListAsset;
        if (this.hasBeenPlacedAlready) {
            return false;
        }
        PrefabListAsset prefabListAsset2 = prefabListAsset = this.prefabListAssetId != null ? PrefabListAsset.getAssetMap().getAsset(this.prefabListAssetId) : null;
        ...
        }, (xi, zi, entityWrappers, t) -> {}, (xi, yi, zi, path, fitHeightmap, inheritSeed, inheritHeightCondition, weights, rotation, t) -> {}, new PrefabBufferCall(new Random(), PrefabRotation.fromRotation(Rotation.None)));
        accessor.release();
        return false;
    }
}
