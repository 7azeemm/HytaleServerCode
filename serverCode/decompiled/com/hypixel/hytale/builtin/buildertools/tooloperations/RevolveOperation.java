...
public class RevolveOperation
extends ToolOperation {
    private Vector3f center;
    private BlockSelection currentSelection;
    private Sampling samplingMode = Sampling.Neighbor;
    ...
    public RevolveOperation(@Nonnull Ref<EntityStore> ref, @Nonnull Player player, @Nonnull BuilderToolOnUseInteraction packet, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (!selection.hasSelectionBounds()) {
        ...
        switch ((String)this.args.tool().getOrDefault("aSampling", "neighbor")) {
            case "reverse": {
                this.samplingMode = Sampling.Reverse;
                break;
            }
            case "none": {
                this.samplingMode = Sampling.Disabled;
                break;
            }
            default: {
                this.samplingMode = Sampling.Neighbor;
            }
        }
        ...
        switch ((String)this.args.tool().getOrDefault("dCenter", "target")) {
            ...
            default: {
                this.center = new Vector3f((float)this.x + 0.5f, this.y + this.originOffsetY, (float)this.z + 0.5f);
            }
        }
        ...
        if (this.samplingMode.equals((Object)Sampling.Neighbor)) {
        ...
        for (int chunkX = chunkXMin; chunkX <= chunkXMax; ++chunkX) {
            for (int chunkZ = chunkZMin; chunkZ <= chunkZMax; ++chunkZ) {
                Object chunk = accessor.getChunk(ChunkUtil.indexChunk(chunkX, chunkZ));
                ...
                for (int x = localXMin; x <= localXMax; ++x) {
                    for (int z = localZMin; z <= localZMax; ++z) {
                        for (int y = yMax; y >= yMin; --y) {
                            ...
                            if (!(block == 0 || filler != 0 || this.edit.getBlockMask() != null && this.edit.getBlockMask().isExcluded(accessor, x, y, z, min, max, block, 0))) {
                            ...
                            if (this.samplingMode == Sampling.Neighbor) {
        ...
        ReferenceArrayList entityRefs = new ReferenceArrayList();
        if (!fullRevolve && copyEntities) {
            World world = componentAccessor.getExternalData().getWorld();
            Store<EntityStore> entityStore = world.getEntityStore().getStore();
            ReferenceArrayList entities = new ReferenceArrayList();
            ...
            if (totalEntities <= 1000) {
                ...
                BuilderToolsPlugin.sendFeedback(Message.translation("server.builderTools.errorTooManyEntities").param("count", 1000), player, NotificationStyle.Warning, componentAccessor);
            }
        }
        if (customDistance > 0 && (naturalDist = Math.sqrt((pivotDX = (selCX = (double)(xMin + xMax) / 2.0) - (double)this.center.x) * pivotDX + (pivotDZ = (selCZ = (double)(zMin + zMax) / 2.0) - (double)this.center.z) * pivotDZ)) > 0.0) {
            double scale = (double)customDistance / naturalDist;
            int newPosX = (int)Math.floor((double)this.center.x + pivotDX * scale) - (xMax - xMin) / 2;
            int newPosZ = (int)Math.floor((double)this.center.z + pivotDZ * scale) - (zMax - zMin) / 2;
            this.currentSelection.setPosition(newPosX, yMin, newPosZ);
            int deltaX = newPosX - xMin;
            int deltaZ = newPosZ - zMin;
            for (Ref entity : entityRefs) {
        ...
        if (fullRevolve) {
            this.samplingMode = Sampling.Disabled;
            double radiusXZ = RevolveOperation.computeRadiusXZ(xMin, xMax, zMin, zMax, this.center);
        ...
        this.stepRadians = Math.toRadians(360.0 / (double)this.copyCount);
        this.stepDegrees = 360.0 / (double)this.copyCount;
    }
    ...
    public void executeAt(int posX, int posY, int posZ, ComponentAccessor<EntityStore> componentAccessor) {
        ...
        for (int i = 0; i <= this.copyCount; ++i) {
            double angle = this.stepRadians * (double)i;
            cos[i] = Math.cos(angle);
            sin[i] = Math.sin(angle);
            degrees[i] = this.stepDegrees * (double)i;
        }
        if (this.samplingMode.equals((Object)Sampling.Reverse)) {
            this.reverseSample();
        ...
        this.currentSelection.forEachBlock((bx, by, bz, blockHolder) -> {
            ...
            double deltaX = (double)(bx + this.currentSelection.getX()) + 0.5 - (double)this.center.x;
            double deltaZ = (double)(bz + this.currentSelection.getZ()) + 0.5 - (double)this.center.z;
            for (int c = 1; c <= this.copyCount; ++c) {
                int newRotation = RevolveOperation.rotateBlock(blockHolder, degrees[c]);
                if (this.samplingMode == Sampling.Neighbor && nextCount > 0) {
                    for (int d = 0; d < offsets.length; ++d) {
                        if ((neighborMask & 1 << d) == 0) continue;
                        double offsetDeltaX = (double)offsets[d][0] * 0.33 + deltaX;
                        double offsetDeltaZ = (double)offsets[d][1] * 0.33 + deltaZ;
                        this.rotate(rotVec, c, offsetDeltaX, by, offsetDeltaZ, sin, cos);
                        placeDelayed.addBlockAtWorldPos(rotVec.x, rotVec.y, rotVec.z, blockHolder.blockId(), newRotation, blockHolder.filler(), blockHolder.supportValue());
                    }
                }
                ...
                alreadyPlaced.addBlockAtWorldPos(rotVec.x, rotVec.y, rotVec.z, blockHolder.blockId(), newRotation, blockHolder.filler(), blockHolder.supportValue());
                this.edit.setMaterial(rotVec.x, rotVec.y, rotVec.z, Material.full(blockHolder.blockId(), newRotation, blockHolder.supportValue(), blockHolder.filler(), blockHolder.holder()));
            }
        });
        ...
        placeDelayed.forEachBlock((bx, by, bz, blockHolder) -> {
            if (!alreadyPlaced.hasBlockAtWorldPos(bx, by, bz)) {
                this.edit.setMaterial(bx, by, bz, Material.full(blockHolder.blockId(), blockHolder.rotation(), blockHolder.supportValue(), blockHolder.filler(), blockHolder.holder()));
            }
        });
        this.rotateEntities(componentAccessor, sin, cos, degrees);
    }
    ...
    private void rotateEntities(ComponentAccessor<EntityStore> componentAccessor, double[] sin, double[] cos, double[] degrees) {
        ...
        this.currentSelection.forEachEntity(entityHolder -> {
            ...
            double deltaX = x + (double)this.currentSelection.getX() - (double)this.center.x;
            double deltaZ = z + (double)this.currentSelection.getZ() - (double)this.center.z;
            for (int c = 1; c < this.copyCount; ++c) {
                ...
                HeadRotation headRotComp = ((Holder)copy).getComponent(HeadRotation.getComponentType());
                if (headRotComp != null) {
                    headRotComp.getRotation().addYaw(yawRad);
                }
                transformComponent.getPosition().assign(rotatedX + (double)this.center.x, y + (double)this.currentSelection.getY(), rotatedZ + (double)this.center.z);
                ((Holder)copy).putComponent(UUIDComponent.getComponentType(), new UUIDComponent(UUID.randomUUID()));
                ((Holder)copy).removeComponent(EntityTrackerSystems.Visible.getComponentType());
                ((Holder)copy).removeComponent(NetworkId.getComponentType());
                Ref<EntityStore> entityRef = new Ref<EntityStore>(entityStore);
                world.execute(() -> RevolveOperation.lambda$rotateEntities$1(entityStore, (Holder)copy, entityRef));
    ...
    private int checkNeighbours(int[][] offsets, int blockID, int x, int y, int z, boolean fluid) {
        ...
        if (this.samplingMode == Sampling.Neighbor) {
            for (int d = 0; d < offsets.length; ++d) {
                ...
                if (neighborX < 0 || neighborX >= this.bufferX || neighborZ < 0 || neighborZ >= this.bufferZ) continue;
                boolean bl = !fluid ? this.materialBuffer[neighborX][y][neighborZ].getBlockId() == blockID : (matches = this.materialBuffer[neighborX][y][neighborZ].getFluidId() == blockID);
                if (!matches) continue;
                nextToSame |= 1 << d;
            }
        }
    ...
    private void rotate(Vector3i v, int c, double x, int y, double z, double[] sin, double[] cos) {
        ...
        v.assign((int)Math.floor(rx + (double)this.center.x), y + this.currentSelection.getY(), (int)Math.floor(rz + (double)this.center.z));
    }

    private void reverseSample() {
        this.currentSelection.setAnchor((int)(this.center.x - (float)this.currentSelection.getX()), (int)this.center.y - this.currentSelection.getY(), (int)(this.center.z - (float)this.currentSelection.getZ()));
        for (int c = 1; c <= this.copyCount; ++c) {
            BlockSelection calcSelection = this.currentSelection.cloneSelection().rotateArbitrary((float)this.stepDegrees * (float)c, 0.0f, 0.0f);
            calcSelection.forEachBlock((bx, by, bz, blockHolder) -> this.edit.setMaterial(bx + this.currentSelection.getX(), by + this.currentSelection.getY(), bz + this.currentSelection.getZ(), Material.full(blockHolder.blockId(), blockHolder.rotation(), blockHolder.supportValue(), blockHolder.filler(), blockHolder.holder())));
            calcSelection.forEachFluid((bx, by, bz, fluid, level) -> this.edit.setMaterial(bx + this.currentSelection.getX(), by + this.currentSelection.getY(), bz + this.currentSelection.getZ(), Material.fluid(fluid, level)));
        }
    }
    ...
    private static double computeRadiusXZ(int xMin, int xMax, int zMin, int zMax, Vector3f hit) {
        ...
        for (int cx : xs) {
            ...
            int n = nArray.length;
            for (int i = 0; i < n; ++i) {
                double deltaX = (double)cx + 0.5 - (double)hit.x;
                int cz = nArray[i];
                double deltaZ = (double)cz + 0.5 - (double)hit.z;
    ...
    private static /* synthetic */ void lambda$rotateEntities$1(Store entityStore, Holder copy, Ref entityRef) {
        entityStore.addEntity(copy, entityRef, AddReason.LOAD);
    }
