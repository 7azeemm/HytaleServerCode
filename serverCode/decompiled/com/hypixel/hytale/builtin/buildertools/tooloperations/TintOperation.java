...
extends ToolOperation {
    ...
    public TintOperation(@Nonnull Ref<EntityStore> ref, @Nonnull Player player, @Nonnull BuilderToolOnUseInteraction packet, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (this.blendMode && this.isHoldingAltModeDown) {
            ...
            String hexColor = ColorParseUtil.toHexString(sampledTint & 0xFFFFFF);
            ...
            try {
                ...
                BuilderToolsPlugin.sendFeedback(Message.translation("server.builderTools.pickedColor").param("color", hexColor), player, NotificationStyle.Success, componentAccessor);
            }
        ...
        this.opacity = (double)((Integer)this.args.tool().getOrDefault("cOpacity", 0)).intValue() / 100.0;
        UUIDComponent uuidComponent = ref.getStore().getComponent(ref, UUIDComponent.getComponentType());
        PrototypePlayerBuilderToolSettings prototypeSettings = (PrototypePlayerBuilderToolSettings)PROTOTYPE_TOOL_SETTINGS.get(uuidComponent.getUuid());
        ...
        for (LongOpenHashSet previousSet : prototypeSettings.getIgnoredPaintOperations()) {
            if (previousSet == this.packedPlacedTinsPositions) continue;
    ...
    private int sampleKernelBlend(int x, int z) {
        ...
        for (int deltaX = -4; deltaX <= 4; ++deltaX) {
            for (int deltaZ = -4; deltaZ <= 4; ++deltaZ) {
                ...
                if (bufferX < 0 || bufferZ < 0 || bufferX >= this.colorBuffer.length || bufferZ >= this.colorBuffer[0].length) continue;
                double sigma = 2.0;
                double weight = Math.exp(-(dist * dist) / (2.0 * sigma * sigma));
                int color = this.colorBuffer[bufferX][bufferZ];
                r += (double)(color >> 16 & 0xFF) * weight;
                g += (double)(color >> 8 & 0xFF) * weight;
                b += (double)(color & 0xFF) * weight;
                totalWeight += weight;
            }
        }
        if (totalWeight == 0.0) {
        ...
        return (int)Math.round(r / totalWeight) << 16 | (int)Math.round(g / totalWeight) << 8 | (int)Math.round(b / totalWeight);
    }
}
