...
import com.hypixel.hytale.builtin.buildertools.tooloperations.FloodOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.LaserPointerOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.LayersOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.NoiseOperation;
...
import com.hypixel.hytale.builtin.buildertools.tooloperations.RevolveOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.ScatterOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.SculptOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.SmoothOperation;
import com.hypixel.hytale.builtin.buildertools.tooloperations.TintOperation;
...
implements TriIntObjPredicate<Void> {
    private static final HytaleLogger LOGGER = HytaleLogger.forEnclosingClass();
    protected static final int RANDOM_MAX = 100;
    @Nonnull
    public static final Map<String, OperationFactory> OPERATIONS = new ConcurrentHashMap<String, OperationFactory>();
    @Nonnull
    public static final Map<UUID, PrototypePlayerBuilderToolSettings> PROTOTYPE_TOOL_SETTINGS = new ConcurrentHashMap<UUID, PrototypePlayerBuilderToolSettings>();
    ...
    public ToolOperation(@Nonnull Ref<EntityStore> ref, @Nonnull BuilderToolOnUseInteraction packet, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        this.transform = ToolOperation.getTransform(ref, args, this.vector, componentAccessor);
        ...
        this.mask = ToolOperation.combineMasks(args, this.builderState.getGlobalMask());
        ...
        Vector3i offsets = ToolOperation.getOffsets(this.shapeRange, this.shapeHeight, originRotation, shapeOrigin, this.transform, this.vector, true);
    ...
    public int getBrushSpacing() {
        ...
        if (spacingValue instanceof Number) {
            return ((Number)spacingValue).intValue();
        }
        return 0;
    }
    ...
    public Transform getBrushRotation(ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (rotationValue instanceof String) {
            String rotationSelection = (String)rotationValue;
    ...
    private BlockPattern getPattern(@Nonnull BuilderToolOnUseInteraction packet, BlockPattern pattern) {
        ...
        if ((this instanceof PaintOperation || this instanceof PaintOperation) && pattern.equals(BlockPattern.EMPTY)) {
    ...
    public void execute(ComponentAccessor<EntityStore> componentAccessor) {
        ToolOperation.executeShapeOperation(this.x, this.y, this.z, this, this.shape, this.shapeRange, this.shapeHeight, this.shapeThickness, this.capped);
    }
    ...
    public void executeAt(int posX, int posY, int posZ, ComponentAccessor<EntityStore> componentAccessor) {
        ...
        ToolOperation.executeShapeOperation(posX, posY, posZ, this, this.shape, this.shapeRange, this.shapeHeight, this.shapeThickness, this.capped);
    }
    ...
    private static Vector3i getOffsets(int width, int height, boolean originRotation, BrushOrigin origin, @Nonnull Transform transform, @Nonnull Vector3i vector, boolean applyBottomOriginFix) {
        ...
        transform.apply(vector);
        int oy = vector.getY();
        int n = origin == BrushOrigin.Center ? 0 : (ox = origin == BrushOrigin.Bottom ? ox : -ox);
        int n2 = origin == BrushOrigin.Center ? 0 : (origin == BrushOrigin.Bottom ? oy + (applyBottomOriginFix ? 1 : 0) : (oy = -oy));
        oz = origin == BrushOrigin.Center ? 0 : (origin == BrushOrigin.Bottom ? oz : -oz);
        return vector.assign(ox, oy, oz);
    }

    private static Transform getTransform(@Nonnull Ref<EntityStore> ref, @Nonnull BuilderTool.ArgData args, @Nonnull Vector3i vector, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        Transform rotate = ToolOperation.getRotation(ref, args, vector, componentAccessor);
        Transform mirror = ToolOperation.getMirror(ref, args, vector, componentAccessor);
        return rotate.then(mirror);
    }

    private static Transform getRotation(@Nonnull Ref<EntityStore> ref, @Nonnull BuilderTool.ArgData args, @Nonnull Vector3i vector, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        Rotation rotation = rotationAngle = angle != null ? Rotation.valueOf((String)angle) : Rotation.None;
    ...
    private static Transform getMirror(@Nonnull Ref<EntityStore> ref, @Nonnull BuilderTool.ArgData args, @Nonnull Vector3i vector, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        BrushAxis mirrorAxis;
        Object axis = args.tool().get("builtin_MirrorAxis");
        BrushAxis brushAxis = mirrorAxis = axis != null ? BrushAxis.valueOf((String)axis) : BrushAxis.None;
    ...
    public static BlockMask combineMasks(@Nullable BuilderTool.ArgData args, @Nullable BlockMask globalMask) {
        BlockMask combinedMask;
        boolean brushInvertMask;
        if (args == null) {
            return globalMask;
        }
        ...
        boolean bl = brushInvertMask = invertMask != null ? (Boolean)invertMask : false;
        if (useBrushMaskCommands) {
            ...
            BlockMask[] parsedMaskCommands = (BlockMask[])Arrays.stream(commands).map(m -> m.split(" ")).map(BlockMask::parse).toArray(BlockMask[]::new);
        ...
        if ((combinedMask = BlockMask.combine(brushMask, brushMaskAbove, brushMaskNot, brushMaskBelow, brushMaskAdjacent, brushMaskNeighbor, globalMask)) != null) {
