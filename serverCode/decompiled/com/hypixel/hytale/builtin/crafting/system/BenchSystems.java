...
public class BenchSystems {
    private static void dropUpgradeItems(CommandBuffer<ChunkStore> commandBuffer, @Nullable BlockType type, BenchBlock benchBlock, int rotation, int x, int y, int z) {
        ...
        Holder[] itemEntityHolders = ItemComponent.generateItemDrops(entityStore, List.of(upgradeItems), dropPosition, Vector3f.ZERO);
    ...
    public static class ProcessingBenchLifecycle
    extends RefSystem<ChunkStore> {
        ...
        public ProcessingBenchLifecycle(@Nonnull ComponentType<ChunkStore, ProcessingBenchBlock> componentType, @Nonnull ComponentType<ChunkStore, BenchBlock> benchBlockComponentType) {
        ...
        public void onEntityRemove(@Nonnull Ref<ChunkStore> ref, @Nonnull RemoveReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            if (combinedItemContainer != null) {
                ...
                World world = commandBuffer.getExternalData().getWorld();
                Store<EntityStore> entityStore = world.getEntityStore().getStore();
                Vector3d dropPosition = new Vector3d((double)blockX + 0.5, localY, (double)blockZ + 0.5);
    ...
    public static class ProcessingBenchTick
    extends EntityTickingSystem<ChunkStore> {
        private final ComponentType<ChunkStore, ProcessingBenchBlock> componentType;
        private final ComponentType<ChunkStore, BenchBlock> benchBlockComponentType;
        private final ComponentType<ChunkStore, BlockModule.BlockStateInfo> blockStateInfoComponentType = BlockModule.BlockStateInfo.getComponentType();

        public ProcessingBenchTick(@Nonnull ComponentType<ChunkStore, ProcessingBenchBlock> componentType, @Nonnull ComponentType<ChunkStore, BenchBlock> benchBlockComponentType) {
            this.componentType = componentType;
            this.benchBlockComponentType = benchBlockComponentType;
        }
        ...
        public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            if (processingBenchBlock.getLastTickGameTime() == null || currentGameTime == null || currentGameTime.equals(processingBenchBlock.getLastTickGameTime())) {
                effectiveDt = 0.0f;
            } else {
                float gameElapsedSeconds = (float)Math.max(0L, currentGameTime.toEpochMilli() - processingBenchBlock.getLastTickGameTime().toEpochMilli()) / 1000.0f;
                effectiveDt = (float)((double)gameElapsedSeconds / WorldTimeResource.getSecondsPerTick(world));
            }
            ...
            if (recipe != null) {
                ...
                List<TestRemoveItemSlotResult> result = processingBenchBlock.getInputContainer().getSlotMaterialsToRemove(inputMaterials, true, true);
                if (result.isEmpty()) {
                    if ("Processing".equals(currentState)) {
                        processingBenchBlock.setBlockInteractionState("default", blockType, world, blockX, localY, blockZ);
                        processingBenchBlock.playSound(processingBench.getFailedSoundEventIndex(), entityStore, blockType, rotationIndex, blockX, localY, blockZ);
            ...
            int completions = processingBenchBlock.advanceProcessing(effectiveDt, entityStore, benchBlock, blockStateInfo, blockX, localY, blockZ, blockType, rotationIndex);
            ...
            if (hasFuelSlots) {
                ...
                if (processingBenchBlock.isActive() && processingBenchBlock.getFuelTime() > 0.0f && fuelSlots != null) {
                    for (int i = 0; i < fuelSlots.length; ++i) {
                        if (processingBenchBlock.getFuelContainer().getItemStack((short)i) == null) continue;
    ...
    public static class OnAddOrRemoved
    extends RefSystem<ChunkStore> {
        private final ComponentType<ChunkStore, BlockModule.BlockStateInfo> blockStateInfoComponentType = BlockModule.BlockStateInfo.getComponentType();
        ...
        public void onEntityAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            BenchSystems.dropUpgradeItems(commandBuffer, blockType, benchBlockComponent, blockChunk.getSectionAtBlockY(y).getRotationIndex(x, y, z), ChunkUtil.worldCoordFromLocalCoord(blockChunk.getX(), x), y, ChunkUtil.worldCoordFromLocalCoord(blockChunk.getZ(), z));
        }
        ...
        public void onEntityRemove(@Nonnull Ref<ChunkStore> ref, @Nonnull RemoveReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            BenchSystems.dropUpgradeItems(commandBuffer, null, benchBlockComponent, 0, ChunkUtil.worldCoordFromLocalCoord(blockChunk.getX(), x), y, ChunkUtil.worldCoordFromLocalCoord(blockChunk.getZ(), z));
        }
