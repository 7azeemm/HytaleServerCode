        ...
        for (int i = 0; i < sections.length && i < 10; ++i) {
            ...
            for (int idx = 0; idx < 32768; ++idx) {
                ...
                if (!ticker.canDemote()) {
                    ...
                    boolean bl = canSpread = ChunkUtil.isBorderBlock(x2, z2) || fluidSectionComponent.getFluidId(x2 - 1, y2, z2) == 0 && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x2 - 1, y2, z2))) || fluidSectionComponent.getFluidId(x2 + 1, y2, z2) == 0 && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x2 + 1, y2, z2))) || fluidSectionComponent.getFluidId(x2, y2, z2 - 1) == 0 && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x2, y2, z2 - 1))) || fluidSectionComponent.getFluidId(x2, y2, z2 + 1) == 0 && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x2, y2, z2 + 1)));
                    if (y2 > 0) {
                        FluidSection fluidSection2;
                        canSpread = ChunkUtil.chunkCoordinate(y2) == ChunkUtil.chunkCoordinate(y2 - 1) ? (canSpread |= fluidSectionComponent.getFluidId(x2, y2 - 1, z2) == 0 && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x2, y2 - 1, z2)))) : (canSpread |= (fluidSection2 = sections[i - 1].getComponent(FluidSection.getComponentType())).getFluidId(x2, y2 - 1, z2) == 0 && !FluidTicker.isSolid(blockMap.getAsset(blockChunkComponent.getBlock(x2, y2 - 1, z2))));
                    }
                    if (!canSpread) {
