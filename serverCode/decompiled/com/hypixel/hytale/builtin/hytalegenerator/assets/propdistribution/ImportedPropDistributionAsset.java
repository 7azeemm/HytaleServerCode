...
public class ImportedPropDistributionAsset
extends PropDistributionAsset {
    @Nonnull
    public static final BuilderCodec<ImportedPropDistributionAsset> CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(ImportedPropDistributionAsset.class, ImportedPropDistributionAsset::new, PropDistributionAsset.ABSTRACT_CODEC).append(new KeyedCodec<String>("Name", Codec.STRING, true), (asset, v) -> {
        asset.name = v;
    ...
    public PropDistribution build(@Nonnull PropDistributionAsset.Argument argument) {
        ...
        PropDistributionAsset asset = ImportedPropDistributionAsset.getExportedAsset(this.name);
        if (asset == null) {
            LoggerUtil.getLogger().warning("Couldn't find Positions asset exported with name: '" + this.name + "'.");
            return NoPropDistribution.INSTANCE;
        }
        PropDistribution out = asset.build(argument);
        return out;
    }
}
