...
extends Assignments {
    ...
    public FieldFunctionAssignments(@Nonnull Density functionTree, @Nonnull List<FieldDelimiter> delimiters) {
        this.density = functionTree;
        this.delimiters = new ArrayList<FieldDelimiter>(delimiters);
    ...
    public Prop propAt(@Nonnull Vector3d position, @Nonnull WorkerIndexer.Id id, double distanceFromBiomeEdge) {
        ...
        for (FieldDelimiter delimiter : this.delimiters) {
            if (!delimiter.isInside(fieldValue)) continue;
    ...
    public List<Prop> getAllPossibleProps() {
        ...
        for (FieldDelimiter f : this.delimiters) {
