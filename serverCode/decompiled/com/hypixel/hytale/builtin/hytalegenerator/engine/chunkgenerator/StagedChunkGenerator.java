...
public class StagedChunkGenerator
implements ChunkGenerator {
    ...
    @Nonnull
    public static final Bounds3i SINGLE_BUFFER_TILE_BOUNDS_BUFFER_GRID = new Bounds3i(new Vector3i(0, 0, 0), new Vector3i(VoxelBuffer.SIZE.x, 320, VoxelBuffer.SIZE.x));
    ...
    public GeneratedChunk generate(@Nonnull ChunkRequest.Arguments arguments) {
        ...
        for (int stageIndex = 0; stageIndex < this.stages.length; ++stageIndex) {
            ...
            ArrayList<BufferBundle.Grid> outputGrids = new ArrayList<BufferBundle.Grid>(outputTypes.size());
            ...
            while (tilePos_bufferGrid.x < stageChunkOutputBounds_bufferGrid.max.x) {
                ...
                while (tilePos_bufferGrid.z < stageChunkOutputBounds_bufferGrid.max.z) {
                    ...
                    for (BufferBundle.Grid grid : outputGrids) {
                        ...
                        if (StagedChunkGenerator.isColumnCached(access, tilePos_bufferGrid, stageIndex)) continue;
    ...
    private Map<BufferType, BufferBundle.Access> createAccesses(@Nonnull Bounds3i localChunkBounds_bufferGrid) {
        HashMap<BufferType, BufferBundle.Access> accessMap = new HashMap<BufferType, BufferBundle.Access>();
        for (int stageIndex = 0; stageIndex < this.stages.length; ++stageIndex) {
            Stage stage = this.stages[stageIndex];
            List<BufferType> outputTypes = stage.getOutputTypes();
    ...
    @Nonnull
    private Runnable createTileTask(int stageIndex, @Nonnull Vector3i position_bufferTileGrid, @Nonnull WorkerIndexer.Id workerId, @Nonnull Map<BufferType, BufferBundle.Access> accessMap) {
        Stage stage = this.stages[stageIndex];
        ...
        Stage.Context context = new Stage.Context(new HashMap<BufferType, BufferBundle.Access.View>(bufferAccessCount), workerId);
        for (Map.Entry<BufferType, Bounds3i> entry : inputTypesAndBounds_tileGrid.entrySet()) {
        ...
        return () -> {
            try {
                ...
                for (BufferType outputType : stage.getOutputTypes()) {
                    StagedChunkGenerator.updateTrackersForColumn(stageIndex, ((BufferBundle.Access)accessMap.get(outputType)).createView(), bufferPositionClone_bufferTileGrid);
                }
            }
        };
    }
    ...
    private CompletableFuture<Void> transferBlockStates(@Nonnull ChunkRequest.Arguments arguments, @Nonnull GeneratedBlockStateChunk blockStateChunk, @Nonnull TimeInstrument.Probe transfer_timeProbe) {
        ...
        VoxelBufferView<Material> materialVoxelSpace = new VoxelBufferView<Material>(materialBufferAccess.createView(), Material.class);
            ...
            while (position_voxelGrid.x < chunkBounds_voxelGrid.max.x) {
                ...
                while (position_voxelGrid.z < chunkBounds_voxelGrid.max.z) {
                    ...
                    while (position_voxelGrid.y < chunkBounds_voxelGrid.max.y) {
                        SolidMaterial solidMaterial = ((Material)materialVoxelSpace.get(position_voxelGrid)).solid();
                ...
                for (int x_voxelGrid = 0; x_voxelGrid < 32; ++x_voxelGrid) {
                    for (int z_voxelGrid = 0; z_voxelGrid < 32; ++z_voxelGrid) {
                        ...
                        for (int y_voxelGrid = minY_voxelGrid; y_voxelGrid < maxY_voxelGrid; ++y_voxelGrid) {
                            ...
                            Material material = (Material)materialVoxelSpace.get(world_voxelGrid);
                            ...
                            StagedChunkGenerator.setSupport(generatedChunk, x_voxelGrid, y_voxelGrid, z_voxelGrid, material.solid().blockId, material.solid().support);
    ...
    private CompletableFuture<Void> transferTints(@Nonnull ChunkRequest.Arguments arguments, @Nonnull GeneratedChunk generatedChunk, @Nonnull TimeInstrument.Probe transfer_timeProbe) {
        ...
        PixelBufferView<Integer> tintVoxelSpace = new PixelBufferView<Integer>(tintBufferAccess.createView(), Integer.class);
        GeneratedBlockChunk blockChunk = generatedChunk.getBlockChunk();
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            for (int x_voxelGrid = 0; x_voxelGrid < 32; ++x_voxelGrid) {
                for (int z_voxelGrid = 0; z_voxelGrid < 32; ++z_voxelGrid) {
                    ...
                    Integer tint = (Integer)tintVoxelSpace.get(worldPosition_voxelGrid);
                    if (tint == null) {
    ...
    private CompletableFuture<Void> transferEnvironments(@Nonnull ChunkRequest.Arguments arguments, @Nonnull GeneratedChunk generatedChunk, @Nonnull TimeInstrument.Probe transfer_timeProbe) {
        ...
        VoxelBufferView<Integer> environmentVoxelSpace = new VoxelBufferView<Integer>(environmentBufferAccess.createView(), Integer.class);
                    ...
                    for (int x_voxelGrid = minX_voxelGrid; x_voxelGrid < maxX_voxelGrid; ++x_voxelGrid) {
                        ...
                        while (z_voxelGrid < maxZ_voxelGrid) {
                            ...
                            columnWriter.intake(y_voxelGrid -> {
                                ...
                                Integer environment = (Integer)environmentVoxelSpace.get(world_voxelGrid);
    ...
    public static class Builder {
        @Nonnull
        public final ParametrizedBufferType MATERIAL_OUTPUT_BUFFER_TYPE = new ParametrizedBufferType("MaterialResult", -1, VoxelBuffer.class, Material.class, () -> new VoxelBuffer<Material>(Material.class));
        @Nonnull
        public final ParametrizedBufferType TINT_OUTPUT_BUFFER_TYPE = new ParametrizedBufferType("TintResult", -3, SimplePixelBuffer.class, Integer.class, () -> new SimplePixelBuffer<Integer>(Integer.class));
        @Nonnull
        public final ParametrizedBufferType ENVIRONMENT_OUTPUT_BUFFER_TYPE = new ParametrizedBufferType("EnvironmentResult", -4, VoxelBuffer.class, Integer.class, () -> new VoxelBuffer<Integer>(Integer.class));
        @Nonnull
        ...
        @Nonnull
        public Builder appendStage(@Nonnull Stage stage) {
        ...
        private static int calculateCapacityFromBounds(@Nonnull Bounds3i bounds, double factor, double viewDistance_voxelGrid, double playerCount) {
            ...
            double viewDistance_bufferGrid = viewDistance_voxelGrid / (double)VoxelBuffer.SIZE.x;
        ...
        private Set<BufferType> createListOfAllBufferTypes() {
            HashSet<BufferType> allBufferTypes = new HashSet<BufferType>();
