...
public class BiomeDistanceStage
implements Stage {
    ...
    public void run(@Nonnull Stage.Context context) {
        ...
        PixelBufferView<BiomeDistanceEntries> biomeDistanceSpace = new PixelBufferView<BiomeDistanceEntries>(biomeDistanceAccess, biomeDistanceClass);
    ...
    @Nonnull
    private BiomeDistanceEntries createDistanceTracker(@Nonnull BufferBundle.Access.View biomeAccess, @Nonnull PixelBufferView<Integer> biomeSpace, @Nonnull Vector3i targetPosition_voxelGrid) {
        ...
        while (position_bufferGrid.x < scanBounds_bufferGrid.max.x) {
            ...
            while (position_bufferGrid.z < scanBounds_bufferGrid.max.z) {
                double distanceToBuffer_voxelGrid = BiomeDistanceStage.distanceToBuffer_voxelGrid(targetPosition_voxelGrid, position_bufferGrid);
                if (!((distanceToBuffer_voxelGrid = Math.max(distanceToBuffer_voxelGrid - 1.0, 0.0)) > this.maxDistance_voxelGrid)) {
                    CountedPixelBuffer biomeBuffer = (CountedPixelBuffer)biomeAccess.getBuffer(position_bufferGrid).buffer();
                    List<Integer> uniqueBiomeIds = biomeBuffer.getUniqueEntries();
                    assert (!uniqueBiomeIds.isEmpty());
                    if (!BiomeDistanceStage.allBiomesAreCountedAndFarther(counter, uniqueBiomeIds, distanceToBuffer_voxelGrid)) {
