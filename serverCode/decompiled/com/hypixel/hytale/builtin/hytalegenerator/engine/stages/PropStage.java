...
implements Stage {
    ...
    public PropStage(@Nonnull String stageName, @Nonnull ParametrizedBufferType biomeInputBufferType, @Nonnull ParametrizedBufferType biomeDistanceInputBufferType, @Nonnull ParametrizedBufferType materialInputBufferType, @Nullable BufferType entityInputBufferType, @Nonnull ParametrizedBufferType materialOutputBufferType, @Nonnull BufferType entityOutputBufferType, @Nonnull MaterialCache materialCache, @Nonnull WorkerIndexer.Data<WorldStructure> worldStructure_workerData, int runtimeIndex) {
        ...
        this.worldStructure_workerData.get(WorkerIndexer.Id.MAIN).getBiomeRegistry().forEach((biomeId, biome) -> allBiomes.add(biome));
        this.materialInputBounds_voxelGrid = new Bounds3i();
        this.positionsBounds_voxelGrid = new Bounds3i();
        for (Biome biome2 : allBiomes) {
            for (PropRuntime propRuntime : biome2.getPropRuntimes()) {
                if (propRuntime.getRuntimeIndex() != this.runtimeIndex) continue;
                propRuntime.getPropDistribution().forEachPossibleProp(prop -> {
                    ...
                    Bounds3i propInputBounds_voxelGrid = PropStage.toInputBounds_voxelGrid(readBounds_voxelGrid, writeBounds_voxelGrid);
                    this.materialInputBounds_voxelGrid.encompass(propInputBounds_voxelGrid);
                    Bounds3i positionsBounds_voxelGrid = PropStage.toPositionsBounds_voxelGrid(writeBounds_voxelGrid);
    ...
    public void run(@Nonnull Stage.Context context) {
        ...
        PixelBufferView<Integer> biomeInputSpace = new PixelBufferView<Integer>(biomeAccess, biomeClass);
        ...
        VoxelBufferView<Material> materialInputSpace = new VoxelBufferView<Material>(materialInputAccess, materialClass);
        BufferBundle.Access.View materialOutputAccess = context.bufferAccess.get(this.materialOutputBufferType);
        VoxelBufferView<Material> materialOutputSpace = new VoxelBufferView<Material>(materialOutputAccess, materialClass);
        ...
        HashSet<Integer> traversedBiomes = new HashSet<Integer>();
        ArrayList<Biome> biomesInBuffer = new ArrayList<Biome>();
        for (int x = localPositionsBounds_voxelGrid.min.x; x < localPositionsBounds_voxelGrid.max.x; ++x) {
            for (int z = localPositionsBounds_voxelGrid.min.z; z < localPositionsBounds_voxelGrid.max.z; ++z) {
                ...
                if (traversedBiomes.contains(biomeId)) continue;
                traversedBiomes.add(biomeId);
                Biome biome = biomeRegistry.getObject(biomeId);
                biomesInBuffer.add(biome);
            }
        }
        HashMap propRuntimeBiomeMap = new HashMap();
        for (Biome biome : biomesInBuffer) {
            biome.getRuntimesWithIndex(this.runtimeIndex, propRuntime -> propRuntimeBiomeMap.put(propRuntime, biome));
        }
        ...
        for (Map.Entry entry : propRuntimeBiomeMap.entrySet()) {
            PropRuntime propRuntime2 = (PropRuntime)entry.getKey();
            Biome biome = (Biome)entry.getValue();
            PropDistribution propDistribution = propRuntime2.getPropDistribution();
            Pipe.Two<Vector3d, Prop> propDistributionPipe = (position_voxelGrid, prop, control) -> {
                ...
                Integer biomeIdAtPosition = (Integer)biomeInputSpace.get(positionX_voxelGrid, 0, positionZ_voxelGrid);
                ...
                propContext.distanceToBiomeEdge = distanceToBiomeEdge = ((BiomeDistanceStage.BiomeDistanceEntries)biomeDistanceSpace.get(position2d_voxelGrid)).distanceToClosestOtherBiome(biomeIdAtPosition);
