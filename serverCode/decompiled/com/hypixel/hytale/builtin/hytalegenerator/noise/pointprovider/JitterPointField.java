...
import com.hypixel.hytale.builtin.hytalegenerator.noise.pointprovider.PointField;
...
extends PointField {
    ...
    public void points2i(@Nonnull Vector2i min, @Nonnull Vector2i max, @Nonnull Consumer<Vector2i> pointsOut) {
        this.points2d(new Vector2d(min.x, (double)min.y), new Vector2d(max.x, (double)max.y), p -> pointsOut.accept(new Vector2i((int)p.x, (int)p.y)));
    }
    ...
    public void points3d(@Nonnull Vector3d min, @Nonnull Vector3d max, @Nonnull Consumer<Vector3d> pointsOut) {
        ...
        for (double x = cellMin.x; x <= cellMax.x + 0.25; x += 1.0) {
            for (double y = cellMin.y; y <= cellMax.y + 0.25; y += 1.0) {
                for (double z = cellMin.z; z <= cellMax.z + 0.25; z += 1.0) {
                    Vector3d point = this.noise.pointFor(this.seed, this.jitter, x, y, z);
                    point.scale(this.scaleUp3d);
                    if (!VectorUtil.isInside(point, min, max)) continue;
    ...
    public void points2d(@Nonnull Vector2d min, @Nonnull Vector2d max, @Nonnull Consumer<Vector2d> pointsOut) {
        ...
        cellMax.y = FastNoiseLite.fastRound(cellMax.y);
        for (double x = cellMin.x; x <= cellMax.x + 0.25; x += 1.0) {
            for (double z = cellMin.y; z <= cellMax.y + 0.25; z += 1.0) {
