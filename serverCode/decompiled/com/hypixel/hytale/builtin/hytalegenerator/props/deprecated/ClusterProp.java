...
extends Prop {
    ...
    public ClusterProp(int range, @Nonnull Double2DoubleFunction weightCurve, int seed, @Nonnull WeightedMap<Prop> propWeightedMap, @Nonnull Pattern pattern, @Nonnull Scanner scanner) {
        ...
        propWeightedMap.forEach((prop, weight) -> {
            if (!this.isColumnBounded((Prop)prop)) {
                return;
            }
    ...
    private boolean isColumnBounded(@Nonnull Prop prop) {
        ...
        return readBounds_voxelGrid.min.x == 0 && readBounds_voxelGrid.min.z == 0 && readBounds_voxelGrid.max.x == 1 && readBounds_voxelGrid.max.z == 1 && writeBounds_voxelGrid.min.x == 0 && writeBounds_voxelGrid.min.z == 0 && writeBounds_voxelGrid.max.x == 1 && writeBounds_voxelGrid.max.z == 1;
    }

    private void place(@Nonnull Vector3i position, @Nonnull VoxelSpace<Material> materialReadSpace, @Nonnull VoxelSpace<Material> materialWriteSpace, @Nonnull EntityFunnel entityBuffer, double distanceFromBiomeEdge) {
        WindowVoxelSpace<Material> columnReadSpace = new WindowVoxelSpace<Material>(materialWriteSpace);
        WindowVoxelSpace<Material> columnWriteSpace = new WindowVoxelSpace<Material>(materialWriteSpace);
        ...
        for (int x = position.x - this.range; x < position.x + this.range; ++x) {
            for (int z = position.z - this.range; z < position.z + this.range; ++z) {
                ...
                if (!materialWriteSpace.getBounds().contains(x, writeBounds_voxelGrid.min.y, z)) continue;
