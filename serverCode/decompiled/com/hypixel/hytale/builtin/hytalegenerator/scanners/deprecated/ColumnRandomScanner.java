...
extends Scanner {
    ...
    public ColumnRandomScanner(int minY, int maxY, int resultsCap, int seed, @Nonnull Strategy strategy, boolean isRelativeToPosition, double baseHeight) {
        ...
        this.rPositions = new ArrayList<Vector3i>();
    }
    ...
    public void scanDartThrow(@NonNullDecl Scanner.Context context) {
        ...
        int range = scanMaxY - (scanMinY = this.isRelativeToPosition ? Math.max(context.position.y + this.minY, bounds.min.y) : Math.max(this.minY, bounds.min.y));
        if (range == 0) {
            return;
        }
        ...
        for (int i = 0; i < numberOfTries; ++i) {
            ...
            if (context.validPositions_out.size() == this.resultsCap) break;
        }
    }
