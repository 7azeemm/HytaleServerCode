...
public class PendingStreamHandler
extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(@Nonnull ChannelHandlerContext ctx, @Nonnull Object msg) {
        if (!(msg instanceof Packet)) {
            LOGGER.at(Level.WARNING).log("Expected Packet but got %s on pending stream from %s", (Object)msg.getClass().getSimpleName(), (Object)NettyUtil.formatRemoteAddress(ctx.channel()));
            ReferenceCountUtil.release(msg);
            ctx.close();
            return;
        }
        Packet packet = (Packet)msg;
        if (!(packet instanceof StreamOpen)) {
            LOGGER.at(Level.WARNING).log("Auxiliary stream first packet was not StreamOpen, closing: %s", packet.getClass().getSimpleName());
            ctx.close();
            return;
        }
        StreamOpen open = (StreamOpen)packet;
        StreamType type = open.type;
        if (this.packetHandler.checkStreamOpenRateLimit()) {
            LOGGER.at(Level.WARNING).log("Stream open rate limited for %s requesting %s", (Object)this.packetHandler.getIdentifier(), (Object)type.name());
        ...
        if (!this.streamManager.isSupported(type)) {
            LOGGER.at(Level.INFO).log("Unsupported stream type %s from %s", (Object)type.name(), (Object)this.packetHandler.getIdentifier());
        ...
        if (existingChannel != null) {
            LOGGER.at(Level.INFO).log("Replacing stale %s stream for %s (old channel active=%s)", type.name(), this.packetHandler.getIdentifier(), existingChannel.isActive());
            this.packetHandler.setChannel(type, null);
            existingChannel.close();
        }
        if (this.packetHandler.getAuxiliaryChannelCount() >= 4) {
            LOGGER.at(Level.WARNING).log("Maximum auxiliary streams exceeded for %s requesting %s", (Object)this.packetHandler.getIdentifier(), (Object)type.name());
        ...
        if (handler == null) {
            LOGGER.at(Level.SEVERE).log("Failed to create handler for stream type %s from %s", (Object)type.name(), (Object)this.packetHandler.getIdentifier());
            ctx.writeAndFlush(new StreamOpenResponse(type, false, "Internal error")).addListener(future -> ctx.close());
            return;
        }
        LOGGER.at(Level.INFO).log("Opening %s stream for %s", (Object)type.name(), (Object)this.packetHandler.getIdentifier());
    ...
    public void exceptionCaught(@Nonnull ChannelHandlerContext ctx, @Nonnull Throwable cause) {
        if (cause instanceof ProtocolException) {
            LOGGER.at(Level.WARNING).log("Protocol error on pending stream for %s: %s", (Object)this.packetHandler.getIdentifier(), (Object)cause.getMessage());
        } else {
