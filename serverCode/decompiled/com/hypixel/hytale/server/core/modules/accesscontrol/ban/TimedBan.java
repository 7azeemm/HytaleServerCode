...
extends AbstractBan {
    ...
    public CompletableFuture<Optional<Message>> getDisconnectReason(@Nonnull UUID uuid) {
        ...
        Message message = this.reason.isPresent() ? Message.translation("client.general.disconnect.banned.timed.withReason").param("days", days).param("hours", hours).param("mins", mins).param("reason", (String)this.reason.get()) : Message.translation("client.general.disconnect.banned.timed").param("days", days).param("hours", hours).param("mins", mins);
