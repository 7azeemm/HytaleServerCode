...
public class BlockEntity {
    public static void setBlockEntity(@Nonnull ComponentAccessor<ChunkStore> accessor, @Nonnull Ref<ChunkStore> chunkRef, @Nonnull BlockComponentChunk componentChunk, int x, int y, int z, @Nonnull BlockType blockType, int rotation, @Nullable Holder<ChunkStore> holder) {
        ...
        if (accessor.getArchetype(chunkRef).contains(ChunkStore.REGISTRY.getNonTickingComponentType())) {
            ...
            FillerBlockUtil.forEachFillerBlock(hitbox, (x1, y1, z1) -> {
                ...
                if (ChunkUtil.isSameChunk(x, z, ox, oz)) {
                    Holder<ChunkStore> otherHolder2 = componentChunk.getEntityHolder(ChunkUtil.indexBlockInColumn(ox, oy, oz));
                    if (otherHolder2 != null) {
                        event.next(ox, oy, oz);
                        accessor.invoke(otherHolder2, event);
                    }
                    return;
                }
                long chunkIndex = ChunkUtil.indexChunkFromBlock(ox, oz);
                Ref<ChunkStore> otherChunk = ((ChunkStore)accessor.getExternalData()).getChunkReference(chunkIndex);
                if (otherChunk == null) {
                    return;
                }
                ...
                if ((otherHolder = otherComponentChunk.getEntityHolder(ChunkUtil.indexBlockInColumn(ox, oy, oz))) != null) {
                    event.next(ox, oy, oz);
