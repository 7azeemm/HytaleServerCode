...
public class ItemContainerBlock
implements Component<ChunkStore> {
    public static final BuilderCodec<ItemContainerBlock> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ItemContainerBlock.class, ItemContainerBlock::new).appendInherited(new KeyedCodec<String>("Droplist", Codec.STRING), (state, o) -> {
        state.droplist = o;
    }, state -> state.droplist, (o, p) -> {
        o.droplist = p.droplist;
    }).addValidator(ItemDropList.VALIDATOR_CACHE.getValidator()).add()).appendInherited(new KeyedCodec<SimpleItemContainer>("ItemContainer", SimpleItemContainer.CODEC), (state, o) -> {
        state.itemContainer = o;
    }, state -> state.itemContainer, (o, p) -> {
        o.itemContainer = p.itemContainer.clone();
    }).add()).appendInherited(new KeyedCodec<Short>("Capacity", Codec.SHORT), (o, i) -> {
        o.capacity = i;
    }, o -> o.capacity, (o, p) -> {
        o.capacity = p.capacity;
    }).add()).build();
    private final transient Map<UUID, ContainerBlockWindow> windows = new ConcurrentHashMap<UUID, ContainerBlockWindow>();
    @Nullable
    protected String droplist;
    protected SimpleItemContainer itemContainer;
    protected short capacity = (short)20;
