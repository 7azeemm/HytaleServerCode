...
import com.hypixel.hytale.server.core.modules.voice.VoiceModuleConfig;
import com.hypixel.hytale.server.core.modules.voice.VoicePacketHandler;
...
extends JavaPlugin {
    ...
    private final Map<UUID, VoicePlayerState> playerStates = new ConcurrentHashMap<UUID, VoicePlayerState>();
    ...
    private final ConcurrentHashMap<UUID, PositionSnapshot> positionCache = new ConcurrentHashMap();
    ...
    protected void setup() {
        ...
        this.getLogger().at(Level.INFO).log("[Voice] VoiceModule initialized (maxDistance=%.1f, refDistance=%.1f, enabled=%s, voiceStreamSupported=true)", Float.valueOf(this.getMaxHearingDistance()), Float.valueOf(this.getReferenceDistance()), this.isVoiceEnabled());
    }
    ...
    private void updateAllPlayerPositions() {
        ...
        HashMap<World, List> playersByWorld = new HashMap<World, List>();
        for (Map.Entry<UUID, VoicePlayerState> entry : this.playerStates.entrySet()) {
            World world;
            ...
            if (playerRef == null || (ref = playerRef.getReference()) == null || (store = ref.getStore()) == null || (externalData = store.getExternalData()) == null || (world = externalData.getWorld()) == null) continue;
            playersByWorld.computeIfAbsent(world, k -> new ArrayList()).add(playerId);
        }
        for (Map.Entry<UUID, VoicePlayerState> entry : playersByWorld.entrySet()) {
            World world = (World)((Object)entry.getKey());
            List playerIds = (List)((Object)entry.getValue());
            world.execute(() -> {
                for (UUID playerId : playerIds) {
                    World freshWorld;
                    EntityStore freshExternalData;
                    Store<EntityStore> freshStore;
                    Ref<EntityStore> freshRef;
                    PlayerRef freshPlayerRef = Universe.get().getPlayer(playerId);
                    if (freshPlayerRef == null || (freshRef = freshPlayerRef.getReference()) == null || !freshRef.isValid() || (freshStore = freshRef.getStore()) == null || (freshExternalData = freshStore.getExternalData()) == null || (freshWorld = freshExternalData.getWorld()) == null) continue;
                    ...
                    this.voiceRouter.updateSpeakerPositionCache(freshPlayerRef, transform.getPosition(), isUnderwater, currentWorldId, networkId, isDead);
                }
            });
        }
    }
    ...
    private void onServerAccessChanged(@Nonnull SingleplayerRequestAccessEvent event) {
        if (event.getAccess() != Access.Private && !this.isVoiceEnabled()) {
            ...
            this.getLogger().at(Level.INFO).log("[Voice] Auto-enabled voice for %s play", (Object)event.getAccess());
    ...
    private void onPlayerConnect(@Nonnull PlayerConnectEvent event) {
        ...
        this.getLogger().at(Level.FINE).log("[Voice] Player connected: %s (%s), totalPlayers=%d", playerRef.getUsername(), playerRef.getUuid(), this.playerStates.size());
    ...
    private void onPlayerDisconnect(@Nonnull PlayerDisconnectEvent event) {
        ...
        this.getLogger().at(Level.FINE).log("[Voice] Player disconnected: %s (%s), stats=[%s], remainingPlayers=%d", playerRef.getUsername(), playerRef.getUuid(), stats, this.playerStates.size());
    }
    ...
    private void broadcastMuteUpdate(@Nonnull UUID playerId, boolean isMuted) {
        this.broadcastConfigToAllPlayers();
        this.getLogger().at(Level.INFO).log("[Voice] Broadcast config update for mute change: playerId=%s, isMuted=%s", (Object)playerId, isMuted);
    }
    ...
    private boolean isEyeInFluid(@Nonnull Vector3d position, @Nonnull World world) {
        ...
        Ref<ChunkStore> chunkRef = chunkStore.getChunkReference(chunkIndex = ChunkUtil.indexChunkFromBlock(blockX, blockZ));
        ...
        return worldChunk.getFluidId(blockX, blockY, blockZ) != 0;
    }
    ...
    public void updatePositionCache(@Nonnull UUID playerId, @Nonnull Vector3d position, boolean isUnderwater, long worldId, int networkId, boolean isDead) {
        this.positionCache.put(playerId, new PositionSnapshot(position.getX(), position.getY() + 1.62, position.getZ(), isUnderwater, worldId, networkId, isDead, System.currentTimeMillis()));
    }

    public PositionSnapshot getCachedPosition(@Nonnull UUID playerId) {
