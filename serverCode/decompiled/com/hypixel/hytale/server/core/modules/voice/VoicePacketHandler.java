...
implements SubPacketHandler {
    ...
    private void handleVoiceData(@Nonnull VoiceData packet) {
        if (!this.loggedGameStreamRejection) {
            ...
            this.logger.at(Level.WARNING).log("[VoicePacket] REJECTED: Voice data received on game stream from %s - client should use dedicated voice stream", playerRef != null ? playerRef.getUsername() : "unknown");
        }
    }
}
