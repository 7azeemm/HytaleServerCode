...
public class BlockOperations {
    ...
    public static void spawnBlockParticles(ChunkStore chunkStore, int oldBlockId, int newBlockId, int x, int y, int z, boolean isPhysics) {
        ...
        if (oldBlockId == 0 && newBlockId != 0) {
            notificationHandler.sendBlockParticle((double)x + 0.5, (double)y + 0.5, (double)z + 0.5, newBlockId, BlockParticleEvent.Build);
            ...
            notificationHandler.sendBlockParticle((double)x + 0.5, (double)y + 0.5, (double)z + 0.5, oldBlockId, particleType);
        } else {
            notificationHandler.sendBlockParticle((double)x + 0.5, (double)y + 0.5, (double)z + 0.5, oldBlockId, BlockParticleEvent.Break);
            notificationHandler.sendBlockParticle((double)x + 0.5, (double)y + 0.5, (double)z + 0.5, newBlockId, BlockParticleEvent.Build);
        }
    }
}
