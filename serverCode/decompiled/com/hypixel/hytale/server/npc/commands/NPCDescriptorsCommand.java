...
public class NPCDescriptorsCommand
extends AbstractAsyncPlayerCommand {
    ...
    protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
        return this.runAsync(context, () -> {
            ...
            context.sendMessage(Message.translation("server.commands.npc.descriptors.saved").param("path", Path.of("npc_descriptors.json", new String[0]).toAbsolutePath().toString()));
        }, world);
    }
}
