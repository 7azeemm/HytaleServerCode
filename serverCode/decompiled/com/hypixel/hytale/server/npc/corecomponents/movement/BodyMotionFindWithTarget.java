...
extends BodyMotionFindBase<AStarWithTarget> {
    ...
    public boolean canComputeMotion(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider infoProvider, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (infoProvider != null && infoProvider.hasPosition() && (positionProvider = infoProvider.getPositionProvider()).providePosition(this.lastTargetPosition)) {
            ...
            if (targetEntityReference != null) {
                ...
                MovementStatesComponent movementStatesComponent = componentAccessor.getComponent(targetEntityReference, MovementStatesComponent.getComponentType());
    ...
    public boolean shouldDeferPathComputation(@Nonnull MotionController motionController, Vector3d position, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (this.throttleCount > this.throttleIgnoreCount && ((distanceSquared = this.getLastTargetPosition().distanceSquaredTo(this.lastPathedPosition)) < 1.0000000000000002E-10 || this.waitForTargetMovement && distanceSquared < this.minMoveDistanceWaitSquared)) {
            return true;
        }
        this.waitForTargetMovement = false;
    ...
    protected Vector3d getLastAccessibleTargetPosition(@Nonnull MotionController motionController, boolean approximate, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (this.dbgMotionState && motionController.is2D()) {
            NPCPlugin.get().getLogger().at(Level.INFO).log("MotionFindWithTarget: Reprojecting %s -> %s", (Object)this.self, (Object)this.other);
        }
        this.lastAccessibleTargetPosition.assign(this.lastTargetPosition);
