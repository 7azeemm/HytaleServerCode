...
public class GroupSteeringAccumulator {
    ...
    public void processEntity(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (NPCPhysicsMath.dotProduct(dx, dy = (yPosition = position.getY()) - this.y, dz = (zPosition = position.getZ()) - this.z, this.componentSelector) < this.maxRangeSquared && NPCPhysicsMath.isInViewCone(this.xViewDirection, this.yViewDirection, this.zViewDirection, this.collisionViewHalfAngleCosine, dx, dy, dz)) {
    ...
    public void processEntity(@Nonnull Ref<EntityStore> ref, double distanceWeight, double positionWeight, double velocityWeight, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        double d = NPCPhysicsMath.dotProduct(dx, dy = position.getY() - this.y, dz = position.getZ() - this.z, this.componentSelector);
        if (d < this.maxRangeSquared && NPCPhysicsMath.isInViewCone(this.xViewDirection, this.yViewDirection, this.zViewDirection, this.collisionViewHalfAngleCosine, dx, dy, dz)) {
            ...
            if (this.normalizeDistances) {
                if (d < 1.0E-12) {
                    while ((d = NPCPhysicsMath.dotProduct(dx = RandomExtra.randomRange(-1.0, 1.0), dy = RandomExtra.randomRange(-1.0, 1.0), dz = RandomExtra.randomRange(-1.0, 1.0))) < 1.0E-12) {
                    }
