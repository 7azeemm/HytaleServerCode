...
extends MotionControllerBase {
    ...
    protected double computeMove(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nonnull Steering steering, double dt, @Nonnull Vector3d translation, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (this.canAct(ref, componentAccessor)) {
            ...
            boolean bl = isMoving = this.moveSpeed * this.moveSpeed + this.climbSpeed * this.climbSpeed > 1.0E-12 && steering.hasTranslation();
            ...
            if (this.debugModeSteer) {
                LOGGER.at(Level.INFO).log("=== Compute = t =%.4f v =%.4f h =%.4f nh=%.4f dh=%.4f", dt, this.moveSpeed, Float.valueOf(57.295776f * heading), Float.valueOf(57.295776f * newHeading), Float.valueOf(57.295776f * turnAngle));
            }
    ...
    protected double executeMove(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Vector3d translation, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (collision == null) {
            ...
            if (!this.moveProbe.probePosition(ref, this.collisionBoundingBox, this.position, this.collisionResult, this.swimDepth, componentAccessor) || canAct && !this.moveProbe.isInWater()) {
                ...
                if (this.debugModeMove) {
                    LOGGER.at(Level.INFO).log("Move - Dive: No Collision, Bisect pos=%s, blocked=%s", (Object)Vector3d.formatShortString(this.position), this.isObstructed);
                }
            } else if (this.debugModeMove) {
                LOGGER.at(Level.INFO).log("Move - Dive: No collision, pos=%s, blocked=%s", (Object)Vector3d.formatShortString(this.position), this.isObstructed);
            }
            if (this.debugModeValidatePositions && !this.isValidPosition(this.position, this.collisionResult, componentAccessor)) {
                throw new IllegalStateException("Invalid position");
            }
        ...
        this.isObstructed = true;
        boolean bl = this.collisionWithSolid = collision.blockMaterial == BlockMaterial.Solid;
        if (!this.moveProbe.probePosition(ref, this.collisionBoundingBox, this.position, this.collisionResult, this.swimDepth, componentAccessor) || canAct && !this.moveProbe.isInWater()) {
    ...
    public double bisect(@Nonnull Ref<EntityStore> ref, @Nonnull Vector3d validPosition, @Nonnull Vector3d invalidPosition, @Nonnull Vector3d result, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        return this.bisect(validPosition, invalidPosition, this, (T _this, Vector3d position) -> _this.probeMoveProbe.probePosition(ref, _this.collisionBoundingBox, (Vector3d)position, _this.collisionResult, _this.swimDepth, componentAccessor), result);
    }
    ...
    public double probeMove(@Nonnull Ref<EntityStore> ref, @Nonnull ProbeMoveData probeMoveData, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (collision == null) {
            ...
            if (!this.probeMoveProbe.probePosition(ref, this.collisionBoundingBox, probePosition, this.collisionResult, this.swimDepth, componentAccessor) || !this.probeMoveProbe.isInWater()) {
                ...
                if (this.debugModeMove) {
                    LOGGER.at(Level.INFO).log("Probe - Dive: No Collision, Bisect pos=%s, distanceLeft=%s", (Object)Vector3d.formatShortString(probePosition), maxDistance - distanceTravelled);
                }
            } else {
                distanceTravelled = maxDistance;
                if (this.debugModeMove) {
                    LOGGER.at(Level.INFO).log("Probe - Dive: No Collision, valid pos=%s, distanceLeft=%s", (Object)Vector3d.formatShortString(probePosition), maxDistance - distanceTravelled);
                }
            }
    ...
    public void updateModelParameters(Ref<EntityStore> ref, Model model, @Nonnull Box boundingBox, ComponentAccessor<EntityStore> componentAccessor) {
        ...
        this.swimDepth = MotionControllerDive.relativeSwimDepthToHeight(ref, this.relativeSwimDepth, model, boundingBox, componentAccessor);
    }
    ...
    public static double relativeSwimDepthToHeight(double swimDepth, @Nullable Box boundingBox, float eyeHeight) {
        ...
        return MotionControllerDive.relativeSwimDepthToBoundingBox(swimDepth, boundingBox, eyeHeight) - boundingBox.getMin().getY();
    }
    ...
    public static double relativeSwimDepthToHeight(@Nullable Ref<EntityStore> ref, double swimDepth, Model model, Box boundingBox, @Nullable ComponentAccessor<EntityStore> componentAccessor) {
        return MotionControllerDive.relativeSwimDepthToHeight(swimDepth, boundingBox, model != null ? model.getEyeHeight(ref, componentAccessor) : 0.0f);
    }
