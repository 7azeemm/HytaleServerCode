...
public class SpawnMarkerBlockStateSystems {
    ...
    private static void createMarker(@Nonnull Ref<ChunkStore> ref, @Nonnull SpawnMarkerBlock state, @Nonnull BlockModule.BlockStateInfo info, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
        ...
        Vector3i pos = new Vector3i(ChunkUtil.worldCoordFromLocalCoord(blockChunk.getX(), bx), by, ChunkUtil.worldCoordFromLocalCoord(blockChunk.getZ(), bz));
        ...
        public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
            ...
            if (chunk != null) {
                ...
                if (blockEntityRef != null && blockEntityRef.getStore().getComponent(blockEntityRef, SpawnMarkerBlock.getComponentType()) == null) {
                    ...
                    commandBuffer.removeEntity(ref, RemoveReason.REMOVE);
                    LOGGER.at(Level.SEVERE).log("Removing block spawn marker due to blockstate mismatch: %s", ref);
                } else {
                ...
                Ref<EntityStore> ref = archetypeChunk.getReferenceTo(index);
                commandBuffer.removeEntity(ref, RemoveReason.REMOVE);
                LOGGER.at(Level.SEVERE).log("Removing block spawn marker due to origin chunk being unloaded: %s", ref);
            }
        }
    }
    ...
    extends EntityTickingSystem<ChunkStore> {
        ...
        public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            if (state.getSpawnMarkerReference().getEntity(store.getExternalData().getWorld().getEntityStore().getStore()) != null) {
                ...
                ref = archetypeChunk.getReferenceTo(index);
                LOGGER.at(Level.SEVERE).log("Creating new spawn marker due to desync with entity: %s", ref);
                SpawnMarkerBlockStateSystems.createMarker(ref, state, info, store.getExternalData().getWorld().getEntityStore().getStore(), commandBuffer);
            }
        }
    }
