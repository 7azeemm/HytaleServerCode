...
extends AbstractWorldCommand {
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
        if (((Boolean)this.environmentsArg.get(context)).booleanValue()) {
            ...
            worldSpawnData.forEachEnvironmentSpawnData(worldEnvironmentSpawnData -> {
                ...
                worldEnvironmentSpawnData.forEachNpcStat((npcIndex, stats) -> {
                    ...
                    double d = failPercent = all > 0 ? MathUtil.percent(all - stats.getSpansSuccess(), all) : 0.0;
                    if (verbose) {
        ...
        if (((Boolean)this.markersArg.get(context)).booleanValue()) {
            AtomicInteger spawnMarkerCount = new AtomicInteger();
            AtomicInteger inactiveSpawnMarkerCount = new AtomicInteger();
            Object2IntOpenHashMap spawnMarkerTypeCounts = new Object2IntOpenHashMap();
            store.forEachChunk(SpawnMarkerEntity.getComponentType(), (archetypeChunk, componentStoreCommandBuffer) -> {
                for (int index = 0; index < archetypeChunk.size(); ++index) {
                    ...
                    if (entity.getSpawnCount() != 0) continue;
                    inactiveSpawnMarkerCount.getAndIncrement();
                }
            });
            AtomicInteger spawnMarkerNPCCount = new AtomicInteger();
            Object2IntOpenHashMap roleCounts = new Object2IntOpenHashMap();
            HashMap<String, Object2IntMap> roleCountsPerMarkerType = new HashMap<String, Object2IntMap>();
            store.forEachChunk(Archetype.of(NPCEntity.getComponentType(), SpawnMarkerReference.getComponentType()), (archetypeChunk, componentStoreCommandBuffer) -> {
                for (int index = 0; index < archetypeChunk.size(); ++index) {
                    ...
                    Ref<EntityStore> markerRef = spawnMarkerReference.getReference().getEntity((ComponentAccessor<EntityStore>)componentStoreCommandBuffer);
                    SpawnMarkerEntity marker = componentStoreCommandBuffer.getComponent(markerRef, SpawnMarkerEntity.getComponentType());
                    Object2IntMap spawnedRoles = roleCountsPerMarkerType.computeIfAbsent(marker.getSpawnMarkerId(), key -> new Object2IntOpenHashMap());
            ...
            roleCountsPerMarkerType.forEach((key, spawnedRoles) -> {
                sb.append("\n  ").append((String)key).append(" (Instances: ").append(spawnMarkerTypeCounts.getInt(key)).append(")");
