...
public class CaveGenerator {
    ...
    public Cave generate(int seed, @Nonnull ChunkGenerator chunkGenerator, @Nonnull CaveType caveType, int x, int y, int z) {
        ...
        FastRandom random = new FastRandom(seedOffset);
    ...
    protected void continueNode(int seed, @Nonnull ChunkGenerator chunkGenerator, @Nonnull Cave cave, @Nonnull CaveNode parent, int depth, @Nonnull Random random) {
        ...
        block0: for (CaveNodeType.CaveNodeChildEntry childEntry : childEntries) {
            int repeat = this.getRepeatCounter(childEntry, random);
            for (int j = 0; j < repeat; ++j) {
                CaveNodeType type;
                if (!this.shouldGenerateChild(childEntry, random)) continue;
                ...
                if (!this.isMatchingHeight(seed, origin, (type = childEntry.getTypes().get(random)).getHeightCondition())) continue;
                ...
                if (!this.isMatchingHeight(seed, shape.getEnd(), type.getHeightCondition())) continue;
                ...
                if (!CaveBiomeMaskFlags.canGenerate(biomeMaskResult)) {
                    if (!CaveBiomeMaskFlags.canContinue(biomeMaskResult)) continue block0;
                    continue;
                }
    ...
    protected PrefabRotation getRotation(@Nonnull CaveNode caveNode) {
        ...
        if (shape instanceof PrefabCaveNodeShape) {
            return ((PrefabCaveNodeShape)shape).getPrefabRotation();
        }
        return null;
    }
    ...
    protected int getNextDepth(@Nonnull CaveNodeType.CaveNodeChildEntry entry, int depth, Random random) {
        ...
        if (entry.getChildrenLimit() != null && (limit = MathUtil.floor(entry.getChildrenLimit().getValue(random))) < nextDepth) {
            return limit;
        }
        return nextDepth;
    }
    ...
    protected void generatePrefab(int seed, @Nonnull ChunkGenerator chunkGenerator, @Nullable CaveNode parent, @Nonnull CaveNode caveNode, @Nonnull CavePrefabContainer.CavePrefabEntry entry, @Nonnull Random random) {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            if (!this.isMatchingBiome(seed, chunkGenerator, config.getBiomeMask(), x, z) || !config.isMatchingNoiseDensity(seed, x, z) || (y = config.getHeight(seed, x, z, caveNode)) == -1 || !config.isMatchingHeight(seed, x, y, z, random) || parent != null && parent.getShape().shouldReplace(seed, x, z, y)) continue;
