...
extends JsonLoader<SeedStringResource, CoverContainer> {
        ...
        try (ListPool.Resource<CoverContainer.CoverContainerEntry> entries = CoverContainer.ENTY_POOL.acquire();){
            if (this.json != null && this.json.isJsonArray()) {
                ...
                for (int i = 0; i < coversArray.size(); ++i) {
                    ...
                    entries.add(new CoverContainerEntryJsonLoader(this.seed.append("-" + i), this.dataFolder, (JsonElement)coversObject).load());
                }
            } else if (this.json != null && this.json.isJsonObject()) {
                JsonObject coversObject = this.json.getAsJsonObject();
                entries.add(new CoverContainerEntryJsonLoader(this.seed.append("-0"), this.dataFolder, (JsonElement)coversObject).load());
            }
            ModifyEvent.SeedGenerator seed = new ModifyEvent.SeedGenerator(this.seed);
            ModifyEvent.dispatch(ModifyEvents.BiomeCovers.class, new ModifyEvents.BiomeCovers(this.biomeContext, entries, content -> new CoverContainerEntryJsonLoader(seed.next(), this.dataFolder, this.getOrLoad(content)).load()));
            CoverContainer coverContainer = new CoverContainer(entries.toArray());
            return coverContainer;
        }
    }

    public static class CoverContainerEntryJsonLoader
    extends JsonLoader<SeedStringResource, CoverContainer.CoverContainerEntry> {
        ...
        protected IWeightedMap<CoverContainer.CoverContainerEntry.CoverContainerEntryPart> loadEntries() {
            ...
            JsonArray jsonArray = weights = this.has("Weight") ? this.get("Weight").getAsJsonArray() : null;
            ...
            WeightedMap.Builder<CoverContainer.CoverContainerEntry.CoverContainerEntryPart> builder = WeightedMap.builder(CoverContainer.CoverContainerEntry.CoverContainerEntryPart.EMPTY_ARRAY);
