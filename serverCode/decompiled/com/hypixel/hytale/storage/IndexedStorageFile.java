...
implements Closeable {
    ...
    public static final MetricsRegistry<IndexedStorageFile> METRICS_REGISTRY = new MetricsRegistry<IndexedStorageFile>().register("Size", file -> {
        try {
            return file.size();
        }
        catch (IOException e) {
    ...
    public static IndexedStorageFile open(@Nonnull Path path, OpenOption ... options) throws IOException {
        return IndexedStorageFile.open(path, 1024, 4096, Set.of(options), NO_ATTRIBUTES);
    }
    ...
    public static IndexedStorageFile open(@Nonnull Path path, @Nonnull Set<? extends OpenOption> options, FileAttribute<?> ... attrs) throws IOException {
        return IndexedStorageFile.open(path, 1024, 4096, options, attrs);
    }
    ...
    public static IndexedStorageFile open(@Nonnull Path path, int blobCount, int segmentSize, OpenOption ... options) throws IOException {
        return IndexedStorageFile.open(path, blobCount, segmentSize, Set.of(options), NO_ATTRIBUTES);
    }
    ...
    public static IndexedStorageFile open(@Nonnull Path path, int blobCount, int segmentSize, @Nonnull Set<? extends OpenOption> options, FileAttribute<?> ... attrs) throws IOException {
        ...
        if (options.contains(StandardOpenOption.CREATE) && storageFile.fileChannel.size() == 0L) {
            ...
            if (storageFile.version == 0) {
                storageFile = IndexedStorageFile.migrateV0(path, blobCount, segmentSize, options, attrs, storageFile);
            } else {
    ...
    protected void writeHeader(int blobCount, int segmentSize) throws IOException {
        ByteBuffer header = IndexedStorageFile.getTempBuffer(HEADER_LENGTH);
    ...
    protected void readUsedSegments() throws IOException {
        ...
        try {
            for (int blobIndex = 0; blobIndex < this.blobCount; ++blobIndex) {
                ...
                if (compressedLength <= 0) continue;
    ...
    public int segmentCount() {
        ...
        try {
            int n = this.usedSegments.cardinality();
            return n;
        }
        finally {
    ...
    public void readBlob(int blobIndex, @Nonnull ByteBuffer dest) throws IOException {
        ...
        if (dest.isDirect()) {
            ...
            ByteBuffer tempDest = IndexedStorageFile.allocateDirect(srcLength);
            try {
    ...
    protected ByteBuffer readBlobHeader(int firstSegmentIndex) throws IOException {
        ...
        ByteBuffer blobHeaderBuffer = IndexedStorageFile.getTempBuffer(BLOB_HEADER_LENGTH);
    ...
    protected ByteBuffer readSegments(int firstSegmentIndex, int compressedLength) throws IOException {
        ...
        ByteBuffer buffer = IndexedStorageFile.allocateDirect(compressedLength);
        if (this.fileChannel.read(buffer, (segmentPosition = this.segmentPosition(firstSegmentIndex)) + (long)BLOB_HEADER_LENGTH) != compressedLength) {
            throw new IllegalStateException();
        }
    ...
    public void writeBlob(int blobIndex, @Nonnull ByteBuffer src) throws IOException {
        block16: {
            ...
            ByteBuffer dest = IndexedStorageFile.allocateDirect(BLOB_HEADER_LENGTH + maxCompressedLength);
            ...
            if (src.isDirect()) {
                ...
                ByteBuffer tempSrc = IndexedStorageFile.allocateDirect(srcLength);
                try {
                    tempSrc.put(src);
                    tempSrc.position(0);
    ...
    private SegmentRangeWriteLock findFreeSegment(int count) {
        ...
        block4: while (true) {
            ...
            try {
                ...
                SegmentRangeWriteLock segmentRangeWriteLock = new SegmentRangeWriteLock(start, count, stamps);
                return segmentRangeWriteLock;
            }
            finally {
